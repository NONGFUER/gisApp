(function(f) {
    ...
    a: function(e, t, n, r) {
        $LMB.register("m_pages_wendaTiwen", function(e, t) {
            var r = n("A");
            var i = r(e, t);
            var s = n("I");
            var o = n("f");
            transTags = o.getTiwenTags;
            transAsk = o.ask;
            var u;
            var a, f;
            var l = t["args"]["cur_city_id"];
            var c = $.queryToJson(location.search.slice(1));
            var h = {
                city_id: l,
                resblock_id: c.resblock_id || ""
            };
            var p = 0;
            var d = {
                init: function() {
                    ...
                    $ljBridge.ready(function(e, t) {
                        ...
                        e.setPageTitle("问答提问");
                        if (t.iswk) {
                            e.actionWithUrl(e.getSchemeLink("web/setbottombar?show=0"))
                        } else {
                ...
                initParam: function() {
                        ...
                        success: function(e) {
                            if (e.errno == 0) {
                                ...
                                for (var n = 0; n < t.length; n++) {
                                    var r = '<a data-act="tab" data-id="' + t[n].id + '">' + t[n].content + "</a>";
                                    $(r).appendTo(".question_type_content")
                                }
                            } else {
                                u.showError("获取标签信息失败，请稍后重试")
                            }
                        },
                        fail: function() {
                            u.showError("获取标签信息失败，请稍后重试")
                    ...
                    var t = location.search ? location.search.substring(1) : "";
                    if (t) {
                        var n = {};
                        n.expertUcid = nodes.one("expertInfo").getData("expert_id");
                        n.expertName = nodes.one("expertInfo").getData("expert_name");
                        if (n.expertUcid && n.expertName) {
                            $(".tiwen_target").find(".green").text(decodeURIComponent(n.expertName));
                            $(".tiwen_target").removeAttr("style");
                            h.expert_ucid = n.expertUcid
                        }
                    }
                    var r = c.resblock_name;
                    if (r) {
                        nodes.one("question_title").attr("placeholder", "#" + decodeURI(r) + "#说点什么”");
                        nodes.one("left_text").text("(" + nodes.one("question_title").val().length + "/30)")
                    }
                },
                evts: {
                    wordCount: function(e) {
                        $(this).siblings("div").text("(" + $(this).val().length + "/30)");
                        if ($(this).val().length != 0) {
                            nodes.one("send").css("background-color", "#00ae66")
                        } else {
                            nodes.one("send").css("background-color", "")
                        }
                    },
                    wordCountForContent: function(e) {
                        $(".num").text($(this).val().length)
                    },
                    turnToTabSelection: function(e) {
                        if (!$.trim(nodes.one("question_title").val())) {
                            u.showError("请输入问题标题");
                            return false
                        }
                        h.title = $("#question_title").val();
                        h.detail = $(".msgwriter-txt").val();
                        $(".question_content").css("display", "none");
                        $(".question_type").css("display", "block");
                        $("body").css("background-color", "white");
                        $(".page").css("background-color", "white")
                    },
                    activeType: function(e) {
                        $(e.el).addClass("active");
                        $(e.el).siblings("a").removeClass("active");
                        nodes.one("submit").css("background-color", "#00ae66");
                        h.tag_id = $(e.el).data("id")
                    },
                    submitQuestion: function(e) {
                        if (!document.cookie.match(/lianjia_token=([^;]+)/)) {
                            if (a) {
                                if (f) {
                                    f.actionLogin("/" + t.args.cur_city_short + "/wenda/tiwen/")
                                }
                            } else {
                                location.href = "/my/login/?redirect=" + encodeURIComponent("/" + t.args.cur_city_short + "/wenda/tiwen/")
                            }
                        } else {
                            if (!p) {
                                if (!h.tag_id) {
                                ...
                                transAsk.request(h, {
                                    success: function(e) {
                                        if (e.errno == 0) {
                                            ...
                                            $("section").css("background-color", "rgb(245,245,245)");
                ...
                bind: function() {
                    de = $("#" + e).de();
                    nodes.one("question_title").on("input", this.evts.wordCount);
                    nodes.one("question_content").on("input", this.evts.wordCountForContent);
                    nodes.one("send").on("click", this.evts.turnToTabSelection);
                    nodes.one("submit").on("click", this.evts.submitQuestion);
                    nodes.one("return_main").on("click", this.evts.returnMain);
                    de.add("tab", "click", this.evts.activeType)
                },
                destroy: function() {}
            };
            d.init();
