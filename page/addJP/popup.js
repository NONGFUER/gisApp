(function(f) {
	...
	a: function(e, t, n, r) {
		$LMB.register("m_pages_gujiaIndex", function(e, t) {
			var r = n("A");
			var i = r(e, t);
			var s = n("I");
			var o = n("f");
			...
			var a;
			var f = $("#" + e).getMark();
			var l = t["args"]["cur_city_id"];
			var c = t["args"]["cur_city_short"];
			var h = t["args"]["gujia_id"];
			var p;
			var d = f.one("feedback_layer");
			var v = f.one("layer_fixed");
			var m = f.one("bottom_layer");
			var g = f.one("layer_list");
			var v = f.one("layer_fixed");
			var y = f.one("bottom_layer_title");
			var b = {};
			var w = [
				["朝向"],
				["东", "east"],
			...
			for(var S = 1; S < w.length; S++) {
				var x = "<li select_data=" + w[S][1] + ">" + w[S][0] + "</li>";
				E.push($(x))
			}
			b.orientationListDom = E;
			var T = [
				["请选择居室数"]
			];
			var N = [
			...
			for(var S = 1; S < T.length; S++) {
				var x = "<li select_data=" + T[S][1] + ">" + T[S][0] + "</li>";
				M.push($(x))
			}
			b.frameListDom = M;
			var _ = [];
			for(var S = 1; S < N.length; S++) {
				var D = "<li select_data=" + N[S][1] + ">" + N[S][0] + "</li>";
				_.push($(D))
			}
			b.hallListDom = _;
			var P = [];
			for(var S = 1; S < C.length; S++) {
				var H = "<li select_data=" + C[S][1] + ">" + C[S][0] + "</li>";
				P.push($(H))
			}
			b.bathListDom = P;
			var B = [
			...
			for(var S = 1; S < B.length; S++) {
				var x = "<li select_data=" + B[S][1] + ">" + B[S][0] + "</li>";
				j.push($(x))
			}
			b.buyHouseIntentionListDom = j;
			var F = [
					["请选择层数"]
				],
				I = [
			...
			for(var S = 1; S < F.length; S++) {
				var x = "<li select_data=" + F[S][1] + ">" + F[S][0] + "</li>";
				U.push($(x))
			}
			b.floorListDom = U;
			var z = [];
			for(var S = 1; S < I.length; S++) {
				var x = "<li select_data=" + I[S][1] + ">" + I[S][0] + "</li>";
				z.push($(x))
			}
			b.floorTotalListDom = z;
			var W = {
				view_result: function(e) {
					var t = {
						community_id: f.one("block_name").attr("select-value"),
						build_size: f.one("build_size").val(),
						frame_bedroom_num: parseInt(f.one("frame_bedroom_num").html()),
						frame_bathroom_num: parseInt(f.one("frame_bathroom_num").html()),
						frame_hall_num: parseInt(f.one("frame_hall_num").html()),
						orientation: f.one("orientation").attr("select-value"),
						floor: f.one("floor_num").attr("select-value"),
						total_floor: f.one("total_floor_num").attr("select-value")
					};
					if(!t["community_id"]) {
						u.showError("请选择小区");
						return false
					...
					if(!t["build_size"]) {
						u.showError("请输入面积");
						return false
					} else if(t["build_size"] <= 0) {
						u.showError("请输入大于0的数字");
						return false
					}
					if(!t["frame_bedroom_num"]) {
						u.showError("请选择卧室个数");
						return false
					}
					if(!t["frame_hall_num"] && t["frame_hall_num"] !== 0) {
						u.showError("请选择厅室个数");
						return false
					}
					if(!t["frame_bathroom_num"] && t["frame_bathroom_num"] !== 0) {
						u.showError("请选择卫生间个数");
						return false
					}
					if(!t["orientation"]) {
						u.showError("请选择朝向");
					...
					if($(".li_item").length == 7) {
						var n = f.one("purchase_intension").attr("select-value");
						if(!n) {
							u.showError("请选择卖房意向");
							return false
						}
					...
					if($(".li_item").length == 7) {
						var r = f.one("mobile").val();
						var i = /^1(3[0-9]|4[57]|5[0-35-9]|7[0135678]|8[0-9])\d{8}$/;
						if(!r) {
							u.showError("请输入手机号码");
							return false
						} else if(!i.test(r)) {
							u.showError("请输入正确格式的手机号码");
							return false
						}
					}
					window["post_ulog"] && window["post_ulog"]("11106", {
						search_condition: {
							"小区": t["community_id"],
							"面积": t["build_size"],
							"户型": t["frame_bedroom_num"] + "室" + t["frame_hall_num"] + "厅" + t["frame_bathroom_num"] + "卫",
							"朝向": t["orientation"],
							"楼层": t["floor"] + "/" + t["total_floor"],
							"买房意向": $(".li_item").length == 7 ? n : "",
							"手机号": $(".li_item").length == 7 ? r : ""
						}
					});
					location.href = "result?community_id=" + t["community_id"] + "&build_size=" + t["build_size"] + "&frame_bedroom_num=" + t["frame_bedroom_num"] + "&frame_bathroom_num=" + t["frame_bathroom_num"] + "&frame_hall_num=" + t["frame_hall_num"] + "&orientation=" + t["orientation"] + "&floor=" + t["floor"] + "&total_floor=" + t["total_floor"]
				},
				mobile_validate: function(e) {
					var t = $(e.el);
					var n = t.val();
					if(n) {
						var r = /^1(3[0-9]|4[57]|5[0-35-9]|7[0135678]|8[0-9])\d{8}$/;
						if(r.test(n)) {
							return
						} else {
							u.showError("请输入正确格式的手机号码")
			...
			var X = {
				orientationList: function(e) {
					v.show();
					y.html(w[0][0]);
					for(var t = 0; t < b.orientationListDom.length; t++) {
						g.append(b.orientationListDom[t])
					}
					g.attr("option-list", "orientationList");
					g.attr("option-list-dom", "orientationListDom");
					m.addClass("active")
				},
				frameList: function(e) {
					v.show();
					y.html(T[0][0]);
					for(var t = 0; t < b.frameListDom.length; t++) {
						g.append(b.frameListDom[t])
					}
					g.attr("option-list", "frameList1");
					g.attr("option-list-dom", "frameListDom");
					m.addClass("active")
				},
				buyHouseIntentionList: function(e) {
					v.show();
					y.html(B[0][0]);
					for(var t = 0; t < b.buyHouseIntentionListDom.length; t++) {
						g.append(b.buyHouseIntentionListDom[t])
					}
					g.attr("option-list", "buyHouseIntentionList");
					g.attr("option-list-dom", "buyHouseIntentionListDom");
					m.addClass("active")
				},
				floorList: function(e) {
					v.show();
					y.html(F[0][0]);
					for(var t = 0; t < b.floorListDom.length; t++) {
						g.append(b.floorListDom[t])
					}
					g.attr("option-list", "floorList");
					g.attr("option-list-dom", "floorListDom");
					m.addClass("active")
				},
				choose_option: function(e) {
					var t = $(e.evt.target);
					var n = t.html();
					var r = t.attr("select_data");
					var i = t.parent("ul").attr("option-list");
					var s = t.parent("ul").attr("option-list-dom");
					var o;
					var u;
					var a = t.index();
					for(var l = 0; l < b[s].length; l++) {
						if(l == a) {
							b[s][l].addClass("active")
						} else {
							b[s][l].removeClass("active")
						}
					}
					t.addClass("active");
					t.siblings().removeClass("active");
					if(i != "frameList1" && i != "frameList2" && i != "floorList") {
						V.bottomCancle();
						f.one(i).find(".chooose").html(n).attr("select-value", r);
						f.one(i).find(".chooose").css("color", "#394043")
					}
					if(i == "frameList1") {
						y.html("请选择厅数");
						g.attr("option-list", "frameList2");
						g.attr("option-list-dom", "hallListDom");
						$(".bedroomChoose").siblings(".please_choose").html("");
						$(".bedroomChoose").parent().css("color", "#394043");
						$(".bedroomChoose").html(n);
						_this = r;
						g.html("");
						for(var c = 0; c < b.hallListDom.length; c++) {
							g.append(b.hallListDom[c])
						}
					}
					if(i == "frameList2") {
						hallNum = r;
						y.html("请选择卫生间数");
						g.attr("option-list", "frameList3");
						g.attr("option-list-dom", "bathListDom");
						$(".hallChoose").html(n);
						g.html("");
						for(var c = 0; c < b.bathListDom.length; c++) {
							g.append(b.bathListDom[c])
						}
					}
					if(i == "frameList3") {
						$(".bathroomChoose").html(n)
					}
					if(i == "floorList") {
						y.html("请选择总楼层数");
						g.attr("option-list", "floorTotalList");
						g.attr("option-list-dom", "floorTotalListDom");
						$(".floorChoose").siblings(".please_choose").html("");
						$(".floorChoose").parent().css("color", "#394043");
						$(".floorChoose").html(n);
						_this = r;
						g.html("");
						f.one("floor_num").attr("select-value", r);
						var h = $(".totalFloorChoose").text();
						if(h) {
							var p = h.substring(h.search(/\d/), h.length - 1);
							if(parseInt(r) > parseInt(p)) {
								$(".totalFloorChoose").html("/ 共" + r + "层");
								f.one("total_floor_num").attr("select-value", r);
								b.floorTotalListDom[r - 1].addClass("active");
								b.floorTotalListDom[parseInt(p) - 1].removeClass("active")
							}
						}
						for(var c = 0; c < b.floorTotalListDom.length; c++) {
							g.append(b.floorTotalListDom[c])
						}
					}
					if(i == "floorTotalList") {
						var d = parseInt($(".floorChoose").html());
						if(d <= parseInt(r)) {
							$(".totalFloorChoose").html("/ 共" + r + "层");
							f.one("total_floor_num").attr("select-value", r)
						} else {
							$(".totalFloorChoose").html("/ 共" + d + "层");
							f.one("total_floor_num").attr("select-value", d);
							b.floorTotalListDom[d - 1].addClass("active");
							b.floorTotalListDom[a].removeClass("active")
						}
					}
				},
				bottom_cancle: function(e) {
					V.bottomCancle()
				}
			};
			var V = {
				...
				bind: function() {
					...
					de.add("view_result", "click", W.view_result);
					de.add("mobile_validate", "change", W.mobile_validate)
				},
				bottomCancle: function() {
